...
const Dashboard: React.FC = () => {
  ...
  const [recognizedFoods, setRecognizedFoods] = useState<FoodNutrition[]>([]);
  ...
  const handleFoodRecognized = (detectedFoods: FoodNutrition[]) => {
  ...
  const addFoodToLog = () => {
    if (recognizedFoods.length > 0) {
      // Add all recognized foods with the current selected date
